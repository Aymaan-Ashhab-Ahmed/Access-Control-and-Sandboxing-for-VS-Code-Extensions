...
**`fs`** fs is a Node standard package which provides POSIX like APIs to interact with the file system [^5]. We choose to wrap fs because if we can sandbox these APIs, many other third-party modules (such as `fs-extra` [^20] and `async-fs-wrapper` [^11]) that rely on `f`s to access file system can be sandboxed as well.
